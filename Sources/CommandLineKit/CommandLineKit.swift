...
public class CommandLineKit {
	...
	*/
	public func parse(strict: Bool = false) throws {
		...
		for (idx, arg) in argumentsEnumerator {
			...
			let splitFlag = flagWithArg.mySplit(by: ArgumentAttacher, maxSplits: 1)
	...
	*/
	...
	#else
	public func printUsage<TargetStream: OutputStreamType>( to: inout TargetStream) {
		/* Nil coalescing operator (??) doesn't work on closures :( */
		let format = formatOutput != nil ? formatOutput! : defaultFormat
	...
	*/
	...
	#else
	public func printUsage<TargetStream: OutputStreamType>(error: ErrorType, to: inout TargetStream) {
		let format = formatOutput != nil ? formatOutput! : defaultFormat
